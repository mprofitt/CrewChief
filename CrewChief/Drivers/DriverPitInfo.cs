...
namespace CrewChief.Drivers
{
    ...
    public class DriverPitInfo
    {
        ...
        public TrackSurfaces PreviousSurface { get; set; } = TrackSurfaces.NotInWorld;
        public TrackSurfaces CurrentSurface { get; set; } = TrackSurfaces.NotInWorld;
        ...
        public void CalculatePitInfo(double time)
        {
            ...
            if (_driver.Live.TrackSurface != CurrentSurface)
            {
                PreviousSurface = CurrentSurface;
                CurrentSurface = _driver.Live.TrackSurface;
            }
            ...
            if (this.PitLaneEntryTime == null)
            {
                ...
                if (this.InPitLane)
                    if (PreviousSurface == TrackSurfaces.OnTrack )
                    {

                        Debug.WriteLine($"\nDriver Name: {_driver.Name}");
                        Debug.WriteLine("Previous TrackSurface: " + PreviousSurface);
                        Debug.WriteLine("TrackSurface: " + _driver.Live.TrackSurface + "\n");

                        // We have only just now entered pitlane
                        this.PitLaneEntryTime = time;
                        this.CurrentPitLaneTimeSeconds = 0;

                        //Place here by me to see if we get PitEntry Event
